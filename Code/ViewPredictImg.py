...
img_file_path = r'/Users/keyanchen/OneDrive/工作/开课吧/无人机课程/第二节/实验/darknet/Code/dataset/Test'
label_file_path = r'/Users/keyanchen/OneDrive/工作/开课吧/无人机课程/第二节/实验/darknet/results/label'
classes = ['balloon']
classToWrite = 0
widthRatio = 1.5
heightRatio = 1.5
...
class ViewPredictImg:
    def __init__(self):
        self.img_file_path = os.path.abspath(img_file_path)
        self.label_file_path = os.path.abspath(label_file_path)
        assert os.path.exists(self.img_file_path), 'img File path dose not exist!'
        assert os.path.exists(self.label_file_path), 'label File path dose not exist!'
        self.class_to_write = classToWrite
        self.widthRatio = widthRatio
        self.heightRatio = heightRatio
        self.scalar = scalar
        self.tempPathName = os.path.abspath(os.path.join(self.img_file_path, r'.\temp'))
    ...
    def readTag(self, filePathName, width, height):
        img_file_path = os.path.splitext(os.path.basename(filePathName))  # 分割出文件扩展名
        txtPathName = os.path.join(self.label_file_path, img_file_path[0] + '.txt')
    ...
    def getFileList(self):
        fileList = []
        list = os.listdir(self.img_file_path)
        for i in range(0, len(list)):
            path = os.path.join(self.img_file_path, list[i])
            if os.path.isfile(path):
                img_file_path = os.path.split(path)  # 分割出目录与文件
                lists = img_file_path[1].split('.')  # 分割出文件与文件扩展名
